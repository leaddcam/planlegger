...
const COOKIE_OPTS = { httpOnly: true, sameSite: 'lax', secure: false, path: '/', maxAge: 1000*60*60*24*7 };
...
transporter.verify().then(() => console.log('smtp ok')).catch(err => console.warn('smtp not ready (dev ok):', err.message));
...
router.get('/verify', async (req, res) => {
  ...
  try {
    ...
    // logg inn og redirect til home
    res.cookie(COOKIE_NAME, jwt.sign({ uid: rec.user_id }, JWT_SECRET, { expiresIn: '7d' }), COOKIE_OPTS);
    res.redirect(`${APP_ORIGIN}/home`);
  } catch (err) {
    console.error(err);
